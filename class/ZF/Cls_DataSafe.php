...
class DataSafe {
     ...
     * @var string
     */
    private static $encrypttype = 'AES-256-ECB';
    /**
     * 默认加密密钥
     * @var string
     */
    private static $encryptkey = 'password';
     ...
     * @return string
     */
    public static function openssl_encrypt($plaintext, $key = '', $type = '') {
        $ret = '';
        if ($key == '') $key = self::$encryptkey;
        if ($type != '') {
            $type = strtoupper($type);
            if (!in_array($type,openssl_get_cipher_methods())) $type = '';
        }
        if ($type =='') {
            $type = self::$encrypttype;
        }
        if ($type && $key && is_string($plaintext) && $plaintext) {
            $ivlen = openssl_cipher_iv_length($type);
            $skey = self::CalcFullLengthKey($key, $ivlen);
            $iv = openssl_random_pseudo_bytes($ivlen);
            $ciphertext_raw = openssl_encrypt($plaintext, $type, $skey, $options = OPENSSL_RAW_DATA, $iv);
     ...
     * @return string
     */
    public static function openssl_decrypt($ciphertext, $key = '', $type = '') {
        $ret = '';
        if ($key == '') $key = self::$encryptkey;
        if ($type != '') {
            $type = strtoupper($type);
        ...
        if ($type && $key && is_string($ciphertext) && $ciphertext) {
            ...
            $original_plaintext = openssl_decrypt($ciphertext_raw, $type, $skey, $options=OPENSSL_RAW_DATA, $iv);
     ...
     * @return bool|string
     */
    private static function CalcFullLengthKey($key,$len) {
        $ret = $key;
    ...
    public static function serializeEncryptVal($val, $key = '', $type = '') {
        return self::openssl_encrypt(serialize($val), $key, $type);
    }
    ...
    public static function serializeDecryptVal($msg, $key = '', $type = '') {
        return unserialize(self::openssl_decrypt($msg, $key, $type));
    }
     ...
     * @return bool
     */
    private static function CheckOpenssl($exit = true) {
        ...
        if (!$ret && $exit) {
            try {
                ...
                exit($e->getFile().':'.$e->getLine().'<br><br> "'.$e->getMessage().'"<br><br><b>Track:</b><br>'.$e->getTraceAsString());
            }
        }
        return $ret;
    }
    ...
    public static function RsaPubEncrypt($cleartext = '', $pub = '') {
        $ret = false;
        if (self::CheckOpenssl() && $pub) {
            $key = openssl_pkey_get_public($pub);
            if ($key && $cleartext) {
                $ary = str_split($cleartext, self::GetEncryptSize($key));
                $ret = '';
                foreach ($ary as $v) {
                    $s = '';
                    if (openssl_public_encrypt($v,$s,$key) === false) return false;
                    $ret .= $s;
                }
                if ($ret) $ret = base64_encode($ret);
            }
        }
        return $ret;
    }
    ...
    public static function RsaPubDecrypt($e = '', $pub = '') {
        ...
        if (self::CheckOpenssl() && $pub) {
            $key = openssl_pkey_get_public($pub);
            if ($key && $e) {
                $ary = str_split(base64_decode($e), self::GetDecryptSize($key));
                $ret = '';
                foreach ($ary as $v) {
                    $s = '';
                    if (openssl_public_decrypt($v,$s,$key) === false) return false;
                    $ret .= $s;
                }
            }
        }
     ...
     * @return int
     */
    private static function GetEncryptSize($key) {
        $ret = 50;
        $ary = openssl_pkey_get_details($key);
        $get_encrypt_size = function($bits=1024){$v = $bits/8-11;return intval($v-$v%50);};
        if (isset($ary) && is_array($ary) && isset($ary['bits'])) $ret = $get_encrypt_size($ary['bits']);
        return $ret;
    }
     ...
     * @return int
     */
    private static function GetDecryptSize($key) {
        $ret = 64;
        $ary = openssl_pkey_get_details($key);
        $get_decrypt_size = function($bits=1024){return intval($bits/8);};
        if (isset($ary) && is_array($ary) && isset($ary['bits'])) $ret = $get_decrypt_size($ary['bits']);
        return $ret;
    }
    ...
    public static function RsaPrvEncrypt($cleartext = '',$prv = '', $prv_pass = '') {
        ...
        if (self::CheckOpenssl() && $prv) {
            ...
            if ($key && $cleartext) {
                ...
                foreach ($ary as $v) {
                    $s = '';
                    if (openssl_private_encrypt($v,$s,$key) === false) return false;
                    $ret .= $s;
                }
                if ($ret) $ret = base64_encode($ret);
            }
    ...
    public static function RsaPrvDecrypt($e='', $prv='', $prv_pass='') {
        ...
        if (self::CheckOpenssl() && $prv) {
            ...
            if ($key && $e) {
                ...
                foreach ($ary as $v) {
                    $s = '';
                    if (openssl_private_decrypt($v,$s,$key) === false) return false;
                    $ret .= $s;
                }
            }
        }
    ...
    public static function CreateNewRsaKey($bits = 2048, $type = 'sha512', $cnf = '') {
        ...
        if ($cnf) {
            if (file_exists($cnf)) $config['config'] = $cnf;
        }
        $ret = false;
        ...
        if ($r) {
            ...
            if ($d && is_array($d) && isset($d['key'])) $pub = $d['key'];
            if ($pub && $prv) {
     ...
     * @return bool|string
     */
    public static function RsaSign($data, $prv = '', $type = OPENSSL_ALGO_SHA256, $pass = '') {
        $ret = false;
        if ($prv) {
            $key = openssl_pkey_get_private($prv, $pass);
            if ($key) {
                openssl_sign($data, $sign, $key, $type);
                if ($sign) $ret = base64_encode($sign);
            }
        }
        return $ret;
    }
     ...
     * @return bool
     */
    public static function RsaVerify($data, $sign, $pub = '', $type = 'sha256WithRSAEncryption') {
        $ret = false;
        if ($sign && $pub) {
     ...
     * @return string
     */
    public static function Xor_Data($str = '', $isbase64 = false) {
