...
class Wepay {
    ...
    public static function CheckSign($ary = array(), $key = '') {
        ...
        if ($ary && is_array($ary)) {
            $type = 'MD5';
            if (isset($ary['sign_type'])) $type = $ary['sign_type'];
            if (isset($ary['sign']) && $ary['sign']) {
                $tmp = $ary;
                unset($tmp['sign']);
                $sign = self::MakeSign($tmp, $key, $type);
                if (hash_equals($ary['sign'],$sign)) $ret = true;
            }
        }
        return $ret;
    }
    ...
    private static function array2xml($data, $cdata = true)
    {
        ...
        foreach ($data as $key => $val) {
            ...
            $xml .= "<$key>";
            $xml .= (is_array($val) || is_object($val)) ? self::array2xml($val) : ($cdata ? self::xmlSafeStr($val) : $val);
     ...
     * @return string
     */
    public static function xml_encode($data, $root = 'xml', $cdata = true, $attr = '', $id = 'id', $encoding = 'utf-8')
    {
        if (is_array($attr)) {
            $_attr = array();
    ...
    public static function unifiedOrder($ary = array(),$openid = '',$isTest = false) {
        ...
        if ($isTest) {
            ...
            $x = self::xmldecode(self::postXmlCurl(self::xmlencode($p),$signurl));

            $msg = 'request='.$signurl.'?'.urldecode(http_build_query($p)).',response='.json_encode($x);
            Common::_savelog('_unifiedorder.txt', $msg, false, true);

            if (isset($x['sandbox_signkey'])) $paykey = $x['sandbox_signkey'];
        }
        ...
        if ($lost) {
            ...
            // 支付参数
            $attach = '';
            if (isset($ary['attach'])) $attach = $ary['attach'];
            $param = array(
            ...
            // 签名算法
            ksort($param);
            $sign = strtoupper(md5(urldecode(http_build_query($param)) . '&key=' . $paykey));
            $param['sign'] = $sign;

            $r = self::xmldecode(self::postXmlCurl(self::xmlencode($param),$url));

            $msg = 'request='.$url.'?'.urldecode(http_build_query($param)).',response='.json_encode($r);
            Common::_savelog('_unifiedorder.txt', $msg, false, true);
            ...
            if ($r['return_code'] != 'SUCCESS') {
                ...
                $rest['signType'] = 'MD5';
                ksort($rest);
                $sign = strtoupper(md5(urldecode(http_build_query($rest)).'&key='.$paykey));
                $rest['paySign'] = $sign;
     ...
     * @return string
     */
    public static function xmlencode($arr)
    {
        $xml = "<xml>";
    ...
    protected static function GetConfig($config = '') {
        if ($config == '' && self::$CONFIG && is_string(self::$CONFIG)) $config = self::$CONFIG;
        return $config;
    }
