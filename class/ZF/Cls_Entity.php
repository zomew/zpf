...
class Entity {
    ...
    public function LoadClass($class, $params = array()) {
        ...
        $name = $class;
        if (is_string($class)) {
            if (isset($this->class[$class])) $class = $this->class[$class];
            if ($tmp = array_search($class, $this->class)) $name = $tmp;
            if ($class == 'ALL') {
            ...
            if ($this->$name && is_object($this->$name)) return $this->$name;
    ...
    private function newInst() {
        ...
        $newClass = function($arg) use ($className) {return new $className($arg);};
