...
class Uploader {
     ...
     * @var string
     */
    private static $thunb_dir = '/uploads/thumb';
    ...
    public static function UploadFiles($tag = '', $createthumb = false, $return = false) {
        ...
        $msg = '请选择需要上传的文件';
        $date = date('Ymd');
        $th = $_SERVER['DOCUMENT_ROOT'] . self::$thunb_dir . '/' . ($tag ? "{$tag}/{$date}/" : '');
        if (!file_exists($th)) @mkdir($th, 0777, true);
            ...
            foreach($_FILES as $v) {
                ...
                if (in_array($type, self::$filetype)) {
                    ...
                    if (self::$sizelimit <= 0 || $v['size'] <= self::$sizelimit) {
                        //$filename = md5(md5($v['name']) . microtime(true) . rand(1, 1000)) . ".{$type}";
                        ...
                        if (file_exists($dir . $filename)) {
                            if ($createthumb) {
                                ...
                                $list[] = array(self::$upload_dir . '/' . ($tag ? "{$tag}/{$date}/" : '') . $filename, self::$thunb_dir . '/' . ($tag ? "{$tag}/{$date}/" : '') . $thumb);
                                $code = 0;
                                $msg = 'success';
                            }else{
                ...
                if (file_exists($v['tmp_name'])) @unlink($v['tmp_name']);
            }
        }

        $ret = array('code' => $code, 'msg' => $msg,);
        if ($list) $ret['list'] = $list;
        if ($return) {
    ...
    public static function CreateThumb($src = '', $dest = '', $type = '', $tw = 0, $th = 0) {
        ...
        if ($src && file_exists($src)) {
            ...
            if ($im) {
                ...
                if ($tw == 0) $tw = self::$thumb_max_width;
                if ($th == 0) $th = self::$thumb_max_height;
                $r = $w / $h;
                if ($r >= $tw / $th) {
                    //原始宽高比大于等行目标宽高比，以宽为主
                    $nw = $tw;
